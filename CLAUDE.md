...
- NOTE: the sections above (Repo Structure, Current Status) describe the
  original single-campaign layout. The repo has since become multi-campaign
  (campaigns/<name>/{data,content,assets,campaign.json,...}, launched via a
  campaign picker) — fail-academy is one of several campaigns now. These
  sections have not been re-audited against that structure; don't trust the
  file counts/paths above without checking the actual campaign folder.

## FAIL Academy — Session 0 Reboot (In Progress)
All planning-related content for this campaign lives under
`campaigns/fail-academy/planning/` — see `reboot_status.md` there for current
status, the design-doc index, and locked-in decisions. Read it before touching
any live data for this campaign. Nothing from the reboot has been applied to
live `/data` yet — don't merge or rewrite live entities without explicit
instruction.
